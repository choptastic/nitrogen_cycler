...
render_element(_Rec = #cycler{class=Class,
    ...
    ?WF_IF(length(Images)>=2, 
        %% only cycle if we have more than one icon
        wf:defer("init_cycler('" ++ ID ++ "', " ++ wf:json_encode(Images) ++ ", " ++ wf:to_list(LingerTime) ++ ")")
    ),

    %% always show the cycler_container, even if it's empty
    #panel{class=[cycler_container, Class], id=ID, style=Style, body=[
        %% If first is empty, then we don't have anything to show, just blank out
        #panel{show_if=(First=/=undefined), class=cycler_card, body=[
            #panel{class=cycler_face, body=[
                #image{image=First}
            ]},
            #panel{show_if=(Second=/=undefined), class=cycler_back, body=[
                #image{image=Second}
            ]}
        ]}
    ]}.
